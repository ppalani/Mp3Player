...
def get_music_attributes(list_of_music_files):
    ...
    for index, val in enumerate(list_of_music_files):
        tag = TinyTag.get(val)
        Music_Attributes.update({index: {'Artist': tag.artist, 'Duration':datetime.timedelta(seconds=round(tag.duration)) , "Path": val,
...
[sg.Text("Browse the song library"),sg.InputText(key='_foldernm_'),sg.FolderBrowse(key='_foldername_'),sg.Button('Load')],
[sg.Table(values=data[:][:], headings=headings, max_col_width=120,def_col_width=20,
                         display_row_numbers=True, justification='center', num_rows=20, alternating_row_color='lightblue', key='_table_')],
[sg.Button('Play',key='_playbut_'),sg.Button('Pause',key='_pause_'),sg.Button('Stop',key='_stop_'),sg.Button('Vol(+)',key='_vplus_'),sg.Button('Vol(-)',key='_vminus_')],
]
...
while True:      
    ...
    if event=='_playbut_':
    	...
    	play_music_track((att[indexsong])["Path"])
    if event=='_pause_':
    	if v_pause:
